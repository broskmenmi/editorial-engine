...
22. Read publication.json and honor automaticPublishingEnabled. When true, let GitHub Actions automatically publish approved changes only to the affected playlist and generate its journey map. When false, preserve manual-only publication. Never toggle the flag without user instruction.
...
Read publication.json and honor automaticPublishingEnabled. When true, let GitHub Actions automatically publish approved changes only to the affected playlist and generate its journey map. When false, preserve manual-only publication. Never toggle the flag without user instruction. After changes, read spotify-status.json and report COMPLETE only for an exact current-ledger match; distinguish pending/failed publication and timestamped last verification on no-change runs. Follow automation.md's five-section response format, include the target playlist's published detailed-site URL from its automation contract or journey-map.json, and append the current generated journey-map SVG after EDITORIAL NOTE. If the compact map is stale, write Journey map updating.
