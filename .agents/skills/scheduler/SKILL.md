...
4. Read `under-review.md` before unrelated scouting or expansion.
...
11. Never use the ChatGPT Spotify connector for playlist search, creation, editing, or publication.
12. Let the GitHub Action publish from the ledger using the persisted playlist ID.
13. Read `spotify-status.json` for publication status and never infer success.
14. Never surface unrelated Spotify playlists or broad fallback results.
