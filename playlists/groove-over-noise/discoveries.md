# GROOVE OVER NOISE — Discovery Log

This file records every editorial run. New entries are appended in reverse chronological order.

## Decision format

### ADD

- **Track:**
- **Artist:**
- **Spotify:**
- **Position:** concrete anchor plus structural role
- **Purpose:** what the track does to the journey
- **Reason:** why it satisfies the constitution

### REVISIT

- **Track:**
- **Artist:**
- **Spotify:**
- **What needs reassessment:**
- **Reassess when:**

### REJECT

- **Track:**
- **Artist:**
- **Spotify:**
- **Why it weakens the identity:**

---

## 2026-07-30 — Daily Run 2

### ADD

- **Track:** Flowing
- **Artist:** Decoder
- **Spotify:** spotify:track:67G0MRoqW60mlqLbWd14Wo
- **BPM:** 138
- **BPM source:** Beatport exact track metadata.
- **Position:** After Vil — AB01 and before Arkan — Nasty Tool, as a provisional third-wave psychological threshold.
- **Purpose:** Open third-wave psychological threshold.
- **Reason:** Psychedelic release context adds contrast without breaking pulse.

### REJECT

- **Track:** Evidence
- **Artist:** Setaoc Mass
- **Spotify:** spotify:track:6B6OOqzZpF6lQsjK2Zav4S
- **BPM:** 137
- **BPM source:** Beatport exact track metadata.
- **Position:** Alternative after AB01 and before Nasty Tool.
- **Purpose:** Test catalogue threshold alternative.
- **Reason:** No evidence distinguishes its role from the accepted threshold.
- **Why it weakens the identity:** Its 136 → 137 → 138 placement is smooth, but no track-specific evidence establishes a distinct function beyond Flowing and Nasty Tool.

### REJECT

- **Track:** U Used to Laugh at My Jokes
- **Artist:** Temudo
- **Spotify:** spotify:track:6G4OuJ8hKbNt96UrmPWI2J
- **BPM:** 140
- **BPM source:** Beatport exact track metadata.
- **Position:** Alternative after AB01 and before Nasty Tool.
- **Purpose:** Test faster threshold alternative.
- **Reason:** Its 136→140→138 path creates avoidable tempo sawtoothing.
- **Why it weakens the identity:** The proposed insertion produces 136 → 140 → 138 → 141, creating an avoidable up-down-up tempo shape immediately before the dominant summit.

### Evidence map

- **Measured evidence:** All three exact Spotify identities and BPM values were resolved; Flowing creates 136 → 138 → 138 → 141 with every adjacent difference within 0–4 BPM.
- **Craft convention:** A restrained threshold before re-entry may distinguish the third wave without creating another crest.
- **Listener report:** No new negative listener evidence exists for the current third wave.
- **Editorial interpretation:** Flowing is provisionally assigned a psychological-threshold role; its exact listening effect remains unconfirmed.

### Audit result

Approved Flowing as the sole addition. It extends the third-wave approach without altering the dominant summit or protected ending. Evidence was vetoed for insufficiently distinct function, and U Used to Laugh at My Jokes was vetoed for tempo sawtoothing. The resulting trajectory is **134 → 136 → 138 → 141 → 145 → 142 → 139 → 140 → 141 → 137 → 136 → 138 → 138 → 141 → 138 → 137 → 134 → 130 → 128 → 125**, with every adjacent difference inside the 0–4 BPM target.

### Canonical ledger after run

1. Rene Wise — Deep Under
2. Skov Bowden — Wurk
3. Ottagone — Ottagone 001
4. Coyu — Cachoeira
5. Maeksll — Two Glasses of Whisky
6. Altinbas — Trail Of
7. Altinbas, Cirkle — Enlightenment
8. Jancen — F Track
9. Maeksll — Tragoidia
10. Marcal — Bionic Jungle
11. Vil — AB01
12. Decoder — Flowing
13. Arkan — Nasty Tool
14. Alarico — Iruka
15. RTNH — Session 4 - Original Mix
16. Stef Mendesidis — Interlynx
17. Maeksll — Minimum
18. Psyk — Eclipse
19. Claudio PRC — Haidou
20. Luigi Tozzi — Calipso

### Spotify reconciliation

Publication is delegated to the Spotify Web API workflow. Exact status must be read from `spotify-status.json` after URI-order verification for the 20-track ledger.

### Editorial note

The third wave now gains psychological contrast before physical re-entry.

---


## 2026-07-23 — Daily Run 1

### ADD

- **Track:** F Track
- **Artist:** Jancen
- **Spotify:** spotify:track:5nfyNPgR6kxvdFQ66PIeEB
- **BPM:** 140
- **BPM source:** Beatport and the exact Spotify track page.
- **Position:** Replace Atlantica Night Ride - GLIA Remix after Altinbas, Cirkle — Enlightenment and before Maeksll — Tragoidia, as the second-wave re-entry.
- **Purpose:** Replace stressful second-wave re-entry.
- **Reason:** Dubby groove replaces busyness while preserving forward motion.

### REVISIT

- **Track:** Antimatter
- **Artist:** Arthur Robert
- **Spotify:** spotify:track:4ingsyWWkWq4LEZhDXicYd
- **BPM:** 140
- **BPM source:** Beatport and the exact Spotify track page.
- **Position:** Alternative replacement for Atlantica after Enlightenment and before Tragoidia.
- **Purpose:** Test a deeper re-entry alternative.
- **Reason:** Deeper restraint may soften re-entry too much.
- **What needs reassessment:** Whether its deeper, calmer movement clearly begins a new wave or leaves Tragoidia to arrive without enough preparation.
- **Reassess when:** Directly compared with Jancen — F Track between Enlightenment and Tragoidia under normal ordered playback.

### REJECT

- **Track:** Conformal Invariance
- **Artist:** Arthur Robert
- **Spotify:** spotify:track:5N914CiNjaNQ1F3qIGEYEv
- **BPM:** 140
- **BPM source:** Beatport and the exact Spotify track page.
- **Position:** Alternative replacement for Atlantica after Enlightenment and before Tragoidia.
- **Purpose:** Test the sparsest re-entry option.
- **Reason:** Its spacious flow continues release instead of rebuilding movement.
- **Why it weakens the identity:** It fits both neighbours numerically, but extends the preceding release rather than clearly returning movement before Tragoidia.

### Removal

- **Track:** Atlantica Night Ride - GLIA Remix
- **Artist:** Kaltblume, Bipolar Disorder Rec., GLIA
- **Spotify:** spotify:track:1uzQ282FuyyakQnpoRbVN3
- **Why removed:** Direct user listening evidence found too much happening, stress during playback, and relief when the track ended; the role was correct but the track failed restraint and immersion.

### Audit result

Approved F Track as the smallest repair for the known second-wave defect. It replaces Atlantica at the same verified 140 BPM, preserving **Enlightenment → re-entry → Tragoidia** as 139 → 140 → 141 while reducing competing activity. Antimatter remains a deeper comparison; Conformal Invariance is rejected because it prolongs release instead of clearly bringing movement back. The complete trajectory remains **134 → 136 → 138 → 141 → 145 → 142 → 139 → 140 → 141 → 137 → 136 → 138 → 141 → 137 → 134 → 130 → 128 → 125**, with every adjacent difference within 0–4 BPM.

### Canonical ledger after run

1. Rene Wise — Deep Under
2. Skov Bowden — Wurk
3. Ottagone — Ottagone 001
4. Coyu — Cachoeira
5. Maeksll — Two Glasses of Whisky
6. Altinbas — Trail Of
7. Altinbas, Cirkle — Enlightenment
8. Jancen — F Track
9. Maeksll — Tragoidia
10. Marcal — Bionic Jungle
11. Vil — AB01
12. Arkan — Nasty Tool
13. Alarico — Iruka
14. Stef Mendesidis — Interlynx
15. Maeksll — Minimum
16. Psyk — Eclipse
17. Claudio PRC — Haidou
18. Luigi Tozzi — Calipso

### Spotify reconciliation

Publication is delegated to the Spotify Web API workflow. Exact status must be read from `spotify-status.json` after URI-order verification for the replacement ledger commit.

### Editorial note

The second wave now returns through steady groove instead of stressful busyness.

---


## 2026-07-22 — Daily Run 1

### REVISIT

- **Track:** Kingpin
- **Artist:** Vinicius Honorio, Ricardo Garduno
- **Spotify:** spotify:track:7fBEw8o7XxHBEsdrjUbROw
- **BPM:** 138
- **BPM source:** Beatport and the exact Spotify track page.
- **Position:** Alternative to Arkan — Nasty Tool after Vil — AB01 and before Alarico — Iruka, as the third-wave re-entry.
- **Purpose:** Compare heavier third-wave re-entry.
- **Reason:** Pressure may rival Iruka instead of preparing it.
- **What needs reassessment:** Whether Kingpin preserves restraint and makes Iruka feel more dominant, rather than creating another crest before the summit.
- **Reassess when:** Directly compared with Nasty Tool and Up & Down after AB01 and before Iruka under normal ordered playback.

### REVISIT

- **Track:** Up & Down
- **Artist:** Arkan
- **Spotify:** spotify:track:0FsRgVOtODhabQaUBhwFga
- **BPM:** 139
- **BPM source:** Beatport and the exact Spotify track page.
- **Position:** Alternative to Arkan — Nasty Tool after Vil — AB01 and before Alarico — Iruka, as the third-wave re-entry.
- **Purpose:** Test unresolved third-wave tension.
- **Reason:** Haunting motion may sharpen Iruka without spending pressure.
- **What needs reassessment:** Whether its unresolved, haunting propulsion remains bodily engaging and prepares Iruka more cleanly without softening the third-wave re-entry.
- **Reassess when:** Directly compared with Nasty Tool and Kingpin after AB01 and before Iruka under normal ordered playback.

### REJECT

- **Track:** Attraction
- **Artist:** Arkan
- **Spotify:** spotify:track:4vQ6qXQBkNa6RlCJt7T50Y
- **BPM:** 135
- **BPM source:** Beatport and the exact Spotify track page.
- **Position:** Alternative to Arkan — Nasty Tool after Vil — AB01 and before Alarico — Iruka, as a softer third-wave re-entry.
- **Purpose:** Test softer third-wave re-entry.
- **Reason:** Its descent creates an unsupported six-BPM rebound.
- **Why it weakens the identity:** Replacing Nasty Tool produces 136 → 135 → 141, so the third wave first retreats and then asks for an unsupported +6 BPM ascent into Iruka.

### Audit result

No additions were approved. The canonical trajectory remains **134 → 136 → 138 → 141 → 145 → 142 → 139 → 140 → 141 → 137 → 136 → 138 → 141 → 137 → 134 → 130 → 128 → 125**, with every existing adjacent difference inside the 0–4 BPM target. Kingpin remains a heavier direct comparison for Nasty Tool, Up & Down enters the revisit queue as a numerically compliant unresolved-tension alternative, and Attraction is rejected for creating a flagged +6 BPM rebound into Iruka. Growth remains paused pending direct listening across **AB01 → Nasty Tool / Kingpin / Up & Down → Iruka → Interlynx → Minimum**.

### Canonical ledger after run

1. Rene Wise — Deep Under
2. Skov Bowden — Wurk
3. Ottagone — Ottagone 001
4. Coyu — Cachoeira
5. Maeksll — Two Glasses of Whisky
6. Altinbas — Trail Of
7. Altinbas, Cirkle — Enlightenment
8. Kaltblume, Bipolar Disorder Rec., GLIA — Atlantica Night Ride - GLIA Remix
9. Maeksll — Tragoidia
10. Marcal — Bionic Jungle
11. Vil — AB01
12. Arkan — Nasty Tool
13. Alarico — Iruka
14. Stef Mendesidis — Interlynx
15. Maeksll — Minimum
16. Psyk — Eclipse
17. Claudio PRC — Haidou
18. Luigi Tozzi — Calipso

### Spotify reconciliation

The canonical 18-track playlist remains COMPLETE with exact URI order, metadata, and cover verification recorded in `spotify-status.json`.

### Editorial note

The summit architecture remains paused until the re-entry comparisons are heard.

---

## 2026-07-21 — Daily Run 1

### REVISIT

- **Track:** Kingpin
- **Artist:** Vinicius Honorio, Ricardo Garduno
- **Spotify:** spotify:track:7fBEw8o7XxHBEsdrjUbROw
- **BPM:** 138
- **BPM source:** Beatport and Spotify-linked SongData metadata.
- **Position:** Alternative to Arkan — Nasty Tool after Vil — AB01 and before Alarico — Iruka, as the third-wave re-entry.
- **Purpose:** Compare third-wave re-entry pressure.
- **Reason:** Equal tempo, but pressure may create another crest.
- **What needs reassessment:** Whether Kingpin preserves restraint and makes Iruka feel more dominant, rather than spending summit pressure before Iruka arrives.
- **Reassess when:** Directly compared with Nasty Tool after AB01 and before Iruka under normal ordered playback.

### REJECT

- **Track:** License To Fly
- **Artist:** Stef Mendesidis
- **Spotify:** spotify:track:4YZ9z4mDt6MTeOOq2z8ZF6
- **BPM:** 145
- **BPM source:** Beatport and Spotify-linked SongBPM metadata.
- **Position:** Alternative to Alarico — Iruka after Nasty Tool and before Interlynx, as the dominant summit.
- **Purpose:** Test a faster summit alternative.
- **Reason:** Its pace creates a prohibited decompression drop.
- **Why it weakens the identity:** The placement produces 138 → 145 → 137, including a prohibited -8 BPM drop into Interlynx while making the summit depend on speed rather than earned pressure.

### REJECT

- **Track:** Black Hawk
- **Artist:** Rene Wise, Vault Records
- **Spotify:** spotify:track:2RnCdXWglVfRc2c3V3rPnS
- **BPM:** 134
- **BPM source:** Beatport metadata and the exact Spotify track page.
- **Position:** Alternative to Maeksll — Minimum after Interlynx and before Eclipse, as the decompression pulse bridge.
- **Purpose:** Test an alternate decompression bridge.
- **Reason:** It duplicates Minimum's decompression bridge function.
- **Why it weakens the identity:** At the same 134 BPM and structural position as Minimum, it adds no new decompression stage and repeats the opening artist without a narrative benefit.

### Audit result

No additions were approved. The existing trajectory remains **134 → 136 → 138 → 141 → 145 → 142 → 139 → 140 → 141 → 137 → 136 → 138 → 141 → 137 → 134 → 130 → 128 → 125**, with every adjacent difference inside the 0–4 BPM target. Kingpin remains a direct comparison for Nasty Tool, while License To Fly and Black Hawk were vetoed for tempo discontinuity and functional duplication. Growth remains paused pending direct listening evidence across **AB01 → Nasty Tool → Iruka → Interlynx → Minimum**.

### Canonical ledger after run

1. Rene Wise — Deep Under
2. Skov Bowden — Wurk
3. Ottagone — Ottagone 001
4. Coyu — Cachoeira
5. Maeksll — Two Glasses of Whisky
6. Altinbas — Trail Of
7. Altinbas, Cirkle — Enlightenment
8. Kaltblume, Bipolar Disorder Rec., GLIA — Atlantica Night Ride - GLIA Remix
9. Maeksll — Tragoidia
10. Marcal — Bionic Jungle
11. Vil — AB01
12. Arkan — Nasty Tool
13. Alarico — Iruka
14. Stef Mendesidis — Interlynx
15. Maeksll — Minimum
16. Psyk — Eclipse
17. Claudio PRC — Haidou
18. Luigi Tozzi — Calipso

### Spotify reconciliation

The canonical 18-track playlist remains COMPLETE with exact URI order, metadata, and cover verification recorded in `spotify-status.json`.

### Editorial note

The summit architecture now requires ears, not another speculative addition.

---

## 2026-07-20 — Daily Run 1

### ADD

- **Track:** Nasty Tool
- **Artist:** Arkan
- **Spotify:** spotify:track:4CDWLgMhxOo57uppdRR690
- **BPM:** 138
- **BPM source:** Beatport and Spotify-linked Mixgraph metadata.
- **Position:** After Vil — AB01 and before Alarico — Iruka, as the third-wave re-entry.
- **Purpose:** Begin the third-wave re-entry.
- **Reason:** Bass-heavy restraint rebuilds pressure without restarting the journey.

### ADD

- **Track:** Iruka
- **Artist:** Alarico
- **Spotify:** spotify:track:3RHjzsJWvCWfrp0QCZHUA2
- **BPM:** 141
- **BPM source:** Spotify, SongData, and Trackify metadata.
- **Position:** After Arkan — Nasty Tool and before Stef Mendesidis — Interlynx, as the dominant main summit.
- **Purpose:** Establish the dominant main summit.
- **Reason:** Raw athletic pressure exceeds every earlier crest without spectacle.

### ADD

- **Track:** Interlynx
- **Artist:** Stef Mendesidis
- **Spotify:** spotify:track:0RQAFOhBjlcD09X62GmiCS
- **BPM:** 137
- **BPM source:** Beatport and Spotify-linked metadata.
- **Position:** After Alarico — Iruka and before Maeksll — Minimum, as the summit decompression entry.
- **Purpose:** Begin the long summit decompression.
- **Reason:** Rolling repetition releases force while preserving bodily immersion.

### Audit result

Approved all three additions as the smallest complete repair for the missing third wave and dominant summit. The resulting trajectory is **134 → 136 → 138 → 141 → 145 → 142 → 139 → 140 → 141 → 137 → 136 → 138 → 141 → 137 → 134 → 130 → 128 → 125**, keeping every adjacent difference within 0–4 BPM. Nasty Tool resolves its revisit uncertainty by serving as re-entry rather than summit; Iruka becomes the dominant summit through combined pressure and density; Interlynx begins the preserved staged decompression.

### Canonical ledger after run

1. Rene Wise — Deep Under
2. Skov Bowden — Wurk
3. Ottagone — Ottagone 001
4. Coyu — Cachoeira
5. Maeksll — Two Glasses of Whisky
6. Altinbas — Trail Of
7. Altinbas, Cirkle — Enlightenment
8. Kaltblume, Bipolar Disorder Rec., GLIA — Atlantica Night Ride - GLIA Remix
9. Maeksll — Tragoidia
10. Marcal — Bionic Jungle
11. Vil — AB01
12. Arkan — Nasty Tool
13. Alarico — Iruka
14. Stef Mendesidis — Interlynx
15. Maeksll — Minimum
16. Psyk — Eclipse
17. Claudio PRC — Haidou
18. Luigi Tozzi — Calipso

### Spotify reconciliation

Publication is delegated to the Spotify Web API workflow. Exact status must be read from `spotify-status.json` after URI-order verification.

### Editorial note

The third wave now earns one summit before a five-stage dissolution.

---

## 2026-07-19 — Daily Run 1

### ADD

- **Track:** AB01
- **Artist:** Vil
- **Spotify:** spotify:track:0MYV5AFY0KOXA5QeJFcNCs
- **BPM:** 136
- **BPM source:** Beatport and Spotify-linked Mixgraph metadata.
- **Position:** After Marcal — Bionic Jungle and before Maeksll — Minimum, as the second-wave release extension.
- **Purpose:** Deepen the second-wave release.
- **Reason:** Bright groove extends release without resetting accumulated pressure.

### REVISIT

- **Track:** Nasty Tool
- **Artist:** Arkan
- **Spotify:** spotify:track:4CDWLgMhxOo57uppdRR690
- **BPM:** 138
- **BPM source:** Beatport and Spotify-linked Mixgraph metadata.
- **Position:** After Vil — AB01 and before Maeksll — Minimum, as a possible restrained third-wave crest.
- **Purpose:** Test a restrained third-wave crest.
- **Reason:** Strong groove lacks a clearly dominant summit function.
- **What needs reassessment:** Whether its bass weight creates a genuinely new crest or only extends the existing release chapter.
- **Reassess when:** Directly compared after AB01 and before Minimum against other 138–140 BPM summit candidates.

### REJECT

- **Track:** Raubtier
- **Artist:** Arkan
- **Spotify:** spotify:track:5tFJO56G80ICGxovcomAFs
- **BPM:** 141
- **BPM source:** Beatport metadata.
- **Position:** After Vil — AB01 and before Maeksll — Minimum, as a stronger crest comparison.
- **Purpose:** Test a forceful third-wave crest.
- **Reason:** Its 141 BPM creates two flagged transitions without evidence.
- **Why it weakens the identity:** It produces 136 → 141 → 134, requiring unsupported +5 and -7 BPM transitions while overstating an unearned crest.

### Audit result

Approved AB01 as the sole addition. It changes **Bionic Jungle → Minimum** from 137 → 134 into 137 → 136 → 134, extending the second-wave release without disturbing the final descent. Nasty Tool remains a direct-listen third-wave comparison; Raubtier was rejected for two flagged transitions and excessive crest pressure.

### Canonical ledger after run

1. Rene Wise — Deep Under
2. Skov Bowden — Wurk
3. Ottagone — Ottagone 001
4. Coyu — Cachoeira
5. Maeksll — Two Glasses of Whisky
6. Altinbas — Trail Of
7. Altinbas, Cirkle — Enlightenment
8. Kaltblume, Bipolar Disorder Rec., GLIA — Atlantica Night Ride - GLIA Remix
9. Maeksll — Tragoidia
10. Marcal — Bionic Jungle
11. Vil — AB01
12. Maeksll — Minimum
13. Psyk — Eclipse
14. Claudio PRC — Haidou
15. Luigi Tozzi — Calipso

### Spotify reconciliation

Publication is delegated to the Spotify Web API workflow. Exact status must be read from `spotify-status.json` after URI-order verification.

### Editorial note

The second-wave release now breathes longer; the dominant summit remains unspent.

---

## 2026-07-18 — Daily Run 3

### ADD

- **Track:** Atlantica Night Ride - GLIA Remix
- **Artist:** Kaltblume, Bipolar Disorder Rec., GLIA
- **Spotify:** spotify:track:1uzQ282FuyyakQnpoRbVN3
- **BPM:** 140
- **BPM source:** Beatport and Spotify-derived metadata.
- **Position:** After Altinbas, Cirkle — Enlightenment and before Maeksll — Tragoidia, as the second-wave re-entry.
- **Purpose:** Begin the second-wave re-entry.
- **Reason:** Spatial propulsion reopens movement without resetting accumulated immersion.

### ADD

- **Track:** Tragoidia
- **Artist:** Maeksll
- **Spotify:** spotify:track:0XQ53FHMmonh9WD6DWVZVb
- **BPM:** 141
- **BPM source:** Beatport and Spotify-derived metadata.
- **Position:** After Atlantica Night Ride - GLIA Remix and before Marcal — Bionic Jungle, as the deeper second crest.
- **Purpose:** Form the deeper second crest.
- **Reason:** Denser hypnosis forms a distinct second crest.

### REJECT

- **Track:** Elevate
- **Artist:** Kashpitzky
- **Spotify:** spotify:track:0K3NQ1b6os764ICvmcWLgf
- **BPM:** 142
- **BPM source:** Beatport and Spotify-derived metadata.
- **Position:** Alternative after Tragoidia as a stronger second crest.
- **Purpose:** Test a stronger second crest.
- **Reason:** Extra pace adds pressure without new narrative function.
- **Why it weakens the identity:** It extends Tragoidia's crest function without introducing a distinct rhythmic, spatial, textural, or psychological chapter.

### Audit result

Approved Atlantica Night Ride - GLIA Remix and Tragoidia as a genuine second wave. The resulting trajectory became **134 → 136 → 138 → 141 → 145 → 142 → 139 → 140 → 141 → 137 → 134 → 130 → 128 → 125**, keeping every adjacent difference within 0–4 BPM while preserving the established final decompression. Elevate was rejected as functional duplication.

### Canonical ledger after run

1. Rene Wise — Deep Under
2. Skov Bowden — Wurk
3. Ottagone — Ottagone 001
4. Coyu — Cachoeira
5. Maeksll — Two Glasses of Whisky
6. Altinbas — Trail Of
7. Altinbas, Cirkle — Enlightenment
8. Kaltblume, Bipolar Disorder Rec., GLIA — Atlantica Night Ride - GLIA Remix
9. Maeksll — Tragoidia
10. Marcal — Bionic Jungle
11. Maeksll — Minimum
12. Psyk — Eclipse
13. Claudio PRC — Haidou
14. Luigi Tozzi — Calipso

### Spotify reconciliation

Publication is delegated to the Spotify Web API workflow. Exact status must be read from `spotify-status.json` after URI-order verification.

### Editorial note

The journey now contains two distinct waves without spending its summit.

---

## 2026-07-18 — Daily Run 2

### ADD

- **Track:** Enlightenment
- **Artist:** Altinbas, Cirkle
- **Spotify:** spotify:track:1LwX0cMaE8susTOodWCMAy
- **BPM:** 139
- **BPM source:** Beatport and Spotify-derived SongBPM metadata.
- **Position:** After Altinbas — Trail Of and before Marcal — Bionic Jungle, as the post-crest pulse bridge.
- **Purpose:** Repair the first release descent.
- **Reason:** Its 139 BPM converts the drop into two controlled steps.

### ADD

- **Track:** Haidou
- **Artist:** Claudio PRC
- **Spotify:** spotify:track:6lUBGdd9KgbFl7E5GSlm2z
- **BPM:** 128
- **BPM source:** Beatport and Spotify-derived SongBPM metadata.
- **Position:** After Psyk — Eclipse and before Luigi Tozzi — Calipso, as the deep decompression bridge.
- **Purpose:** Repair the final decompression descent.
- **Reason:** Deep hypnosis divides the closing drop without reopening pressure.

### REJECT

- **Track:** Lyra I
- **Artist:** Claudio PRC, Luigi Tozzi
- **Spotify:** spotify:track:6P0rprIRxsc3aVL0MjfyuJ
- **BPM:** 128
- **BPM source:** Beatport and Spotify-derived SongBPM metadata.
- **Position:** Alternative closing bridge after Eclipse and before Calipso.
- **Purpose:** Test a longer closing bridge.
- **Reason:** Its duration and drive overstate the established dissolution.
- **Why it weakens the identity:** It duplicates Haidou's bridge function and foregrounds Calipso's artist identity instead of disappearing into the ending.

### Audit result

Approved Enlightenment and Haidou as the smallest repairs for both remaining -5 BPM transitions. The resulting trajectory is **134 → 136 → 138 → 141 → 145 → 142 → 139 → 137 → 134 → 130 → 128 → 125**, keeping every adjacent difference within 0–4 BPM while preserving the established closer. Lyra I was rejected because its long driving form overstates the terminal chapter and duplicates the approved 128 BPM bridge role.

### Canonical ledger after run

1. Rene Wise — Deep Under
2. Skov Bowden — Wurk
3. Ottagone — Ottagone 001
4. Coyu — Cachoeira
5. Maeksll — Two Glasses of Whisky
6. Altinbas — Trail Of
7. Altinbas, Cirkle — Enlightenment
8. Marcal — Bionic Jungle
9. Maeksll — Minimum
10. Psyk — Eclipse
11. Claudio PRC — Haidou
12. Luigi Tozzi — Calipso

### Spotify reconciliation

Publication is delegated to the Spotify Web API workflow. Exact status must be read from `spotify-status.json` after URI-order verification.

### Editorial note

The first wave now descends in controlled stages; growth must re-enter before it.

---

## 2026-07-17 — Daily Run 1

### ADD

- **Track:** Ottagone 001
- **Artist:** Ottagone
- **Spotify:** spotify:track:2DMBWNzWBrjaYHPdSf7HQA
- **BPM:** 138
- **Position:** After Skov Bowden — Wurk and before Coyu — Cachoeira, as the opening pulse bridge.
- **Purpose:** Repair the opening pulse climb.
- **Reason:** Restrained propulsion divides the opening jump into compliant steps.

### REVISIT

- **Track:** Kingpin
- **Artist:** Vinicius Honorio, Ricardo Garduno
- **Spotify:** spotify:track:7fBEw8o7XxHBEsdrjUbROw
- **BPM:** 138
- **Position:** Alternative opening bridge between Wurk and Cachoeira.
- **Purpose:** Test a heavier opening bridge.
- **Reason:** Its pressure may spend the first crest too early.
- **What needs reassessment:** Whether its physical weight reinforces groove or prematurely dominates the opening chapter.
- **Reassess when:** Directly compared with Ottagone 001 between Wurk and Cachoeira.

### REJECT

- **Track:** Agartha
- **Artist:** Wata Igarashi
- **Spotify:** spotify:track:5phzJWdxTLmk8Yd7KOTxSH
- **BPM:** 130
- **Position:** Opening bridge comparison between Wurk and Cachoeira.
- **Purpose:** Test spatial contrast in the opening.
- **Reason:** Its 130 BPM creates a prohibited eleven-beat rebound.
- **Why it weakens the identity:** It turns 136 → 141 into 136 → 130 → 141, creating both an opening drop and a prohibited rebound.

### Audit result

Approved Ottagone 001 as the sole repair. It changes **Wurk → Cachoeira** from +5 BPM inside the opening three into +2 and +3 BPM steps. Kingpin remains a direct-listen comparison because its pressure may overstate the opening; the original Agartha was rejected for numeric and perceived-pulse discontinuity.

### Canonical ledger after run

1. Rene Wise — Deep Under
2. Skov Bowden — Wurk
3. Ottagone — Ottagone 001
4. Coyu — Cachoeira
5. Maeksll — Two Glasses of Whisky
6. Altinbas — Trail Of
7. Marcal — Bionic Jungle
8. Maeksll — Minimum
9. Psyk — Eclipse
10. Luigi Tozzi — Calipso

### Spotify reconciliation

Publication is delegated to the Spotify Web API workflow. Exact status must be read from `spotify-status.json` after URI-order verification.

### Editorial note

The opening now climbs by bodily accumulation rather than metadata tolerance.

---

## 2026-07-16 — Restart Run 4

### ADD

- **Track:** Minimum
- **Artist:** Maeksll
- **Spotify:** spotify:track:31FaOVEATLY9PzDv8YPbaI
- **BPM:** 134
- **Position:** After Marcal — Bionic Jungle and before Psyk — Eclipse, as a pulse-continuity release bridge.
- **Purpose:** Smooth release into focused decompression.
- **Reason:** It repairs two neighbours without reopening pressure.

### REVISIT

- **Track:** Tragoidia
- **Artist:** Maeksll
- **Spotify:** spotify:track:0XQ53FHMmonh9WD6DWVZVb
- **BPM:** 141
- **Position:** Future re-entry chapter before final decompression.
- **Purpose:** Test a deeper second-wave crest.
- **Reason:** Strong re-entry, but repeats the current crest artist.
- **What needs reassessment:** Whether a later Maeksll crest reveals new pressure or merely reprises Two Glasses of Whisky.
- **Reassess when:** Compared against non-Maeksll re-entry candidates after the playlist exceeds one hour.

### REJECT

- **Track:** Tonal
- **Artist:** Claudio PRC
- **Spotify:** spotify:track:04fYJHkqNYRGb1szqgpJYd
- **BPM:** 125
- **Position:** Alternative closer replacing Luigi Tozzi — Calipso.
- **Purpose:** Test a firmer terminal pulse.
- **Reason:** It duplicates closure without repairing any transition.
- **Why it weakens the identity:** Its 125 BPM terminal role duplicates Calipso and adds no new chapter function.

### Audit result

Approved Minimum as the sole repair. It changes **Bionic Jungle → Eclipse** from a -7 BPM transition into -3 and -4 BPM steps. Tragoidia remains a future second-wave candidate; Tonal was rejected as redundant closure.

### Canonical ledger after run

1. Rene Wise — Deep Under
2. Skov Bowden — Wurk
3. Coyu — Cachoeira
4. Maeksll — Two Glasses of Whisky
5. Altinbas — Trail Of
6. Marcal — Bionic Jungle
7. Maeksll — Minimum
8. Psyk — Eclipse
9. Luigi Tozzi — Calipso

### Spotify reconciliation

Publication is delegated to the Spotify Web API workflow. Exact status must be read from `spotify-status.json` after URI-order verification.

### Editorial note

The first wave is repaired; future growth must begin before the final descent.

---

## 2026-07-16 — Restart Run 3

### ADD

- **Track:** Trail Of
- **Artist:** Altinbas
- **Spotify:** spotify:track:1BLpNBx8502KRE2TommLp6
- **Position:** After Skov Bowden — Wurk, as restrained post-peak continuation.
- **Purpose:** Sustain motion while releasing pressure.
- **Reason:** Restrained percussion releases force without flattening momentum.

### ADD

- **Track:** Eclipse
- **Artist:** Psyk
- **Spotify:** spotify:track:6mNUIvYYV5JJVlusXEzLw6
- **Position:** After Altinbas — Trail Of, as focused rhythmic decompression.
- **Purpose:** Narrow rhythm without breaking immersion.
- **Reason:** Disciplined repetition reduces pressure through subtraction.

### ADD

- **Track:** Calipso
- **Artist:** Luigi Tozzi
- **Spotify:** spotify:track:15QxVy6d8RrbPKmsYyuVbT
- **Position:** After Psyk — Eclipse, as deep closing dissolution.
- **Purpose:** Complete the final descent.
- **Reason:** Patient motion dissolves pressure while preserving pulse.

### Audit result

Approved all three additions. Trail Of resolves the stored revisit question, Eclipse continues decompression without a reset, and Calipso closes the arc without functional duplication.

### Canonical ledger after run

1. Rene Wise — Deep Under
2. Maeksll — Two Glasses of Whisky
3. Coyu — Cachoeira
4. Marcal — Bionic Jungle
5. Skov Bowden — Wurk
6. Altinbas — Trail Of
7. Psyk — Eclipse
8. Luigi Tozzi — Calipso

### Spotify reconciliation

Publication is delegated to the Spotify Web API workflow. Exact status must be read from `spotify-status.json` after URI-order verification.

### Editorial note

The complete arc now moves from immersion through pressure into disciplined dissolution.

---

## 2026-07-16 — Restart Run 2

### ADD

- **Track:** Bionic Jungle
- **Artist:** Marcal
- **Spotify:** spotify:track:0FuxLvhDsdGu6XpnxGQrAf
- **Position:** After Coyu — Cachoeira, as the controlled density bridge.
- **Purpose:** Tighten rhythm before pressure rises.
- **Reason:** Layered motion increases density without theatrical escalation.

### ADD

- **Track:** Wurk
- **Artist:** Skov Bowden
- **Spotify:** spotify:track:4N1qAnhFro9fQdBON4Aqxn
- **Position:** After Marcal — Bionic Jungle, as controlled physical pressure.
- **Purpose:** Establish force without aggression.
- **Reason:** Physical percussion peaks through groove rather than spectacle.

### REVISIT

- **Track:** Trail Of
- **Artist:** Altinbas
- **Spotify:** spotify:track:1BLpNBx8502KRE2TommLp6
- **Position:** After Skov Bowden — Wurk, as restrained post-peak continuation.
- **Purpose:** Sustain motion while reducing pressure.
- **Reason:** Its role becomes testable only after pressure settles.
- **What needs reassessment:** Whether it releases Wurk's pressure without flattening momentum.
- **Reassess when:** Audited directly after Wurk against other restrained continuations.

### Audit result

Approved Bionic Jungle and Wurk in that order. Trail Of remains unresolved because the post-peak transition needs direct comparison before admission.

### Canonical ledger after run

1. Rene Wise — Deep Under
2. Maeksll — Two Glasses of Whisky
3. Coyu — Cachoeira
4. Marcal — Bionic Jungle
5. Skov Bowden — Wurk

### Spotify reconciliation

Publication is delegated to the Spotify Web API workflow. Exact status must be read from `spotify-status.json` after URI-order verification.

### Editorial note

The opening now reaches controlled pressure; the next decision must shape release.

---

## 2026-07-16 — Restart Run 1

### ADD

- **Track:** Deep Under
- **Artist:** Rene Wise
- **Spotify:** spotify:track:0yI5YgKcF2mXo3NRlXS660
- **Position:** First, as the opening immersion layer.
- **Purpose:** Establish patience before explicit groove.
- **Reason:** Subtle repetition creates movement without demanding attention.

### ADD

- **Track:** Two Glasses of Whisky
- **Artist:** Maeksll
- **Spotify:** spotify:track:1xbqhGI3Xgntoy56FA91D4
- **Position:** After Rene Wise — Deep Under, as the deep-groove continuation.
- **Purpose:** Deepen movement without increasing spectacle.
- **Reason:** Restrained propulsion extends immersion through patient rhythmic development.

### ADD

- **Track:** Cachoeira
- **Artist:** Coyu
- **Spotify:** spotify:track:6uOhAWYxScF7I867ui1a0V
- **Position:** After Maeksll — Two Glasses of Whisky, as the percussive expansion.
- **Purpose:** Introduce bodily rhythm without abrupt escalation.
- **Reason:** Layered percussion increases motion while preserving continuity.

### Audit result

Approved all three additions. The sequence establishes immersion, sustained groove, and measured percussive expansion without duplicate roles or an abrupt peak.

### Canonical ledger after run

1. Rene Wise — Deep Under
2. Maeksll — Two Glasses of Whisky
3. Coyu — Cachoeira

### Spotify reconciliation

A new Spotify playlist was created at `spotify:playlist:3ZGhDUEr4HzlbFmSXkRhhr`, but Spotify classified the result as generative. Exact contents and order could not be read back through the available connection, so publication is PARTIAL until manually verified against the canonical ledger. No future workflow may treat this playlist as synchronized without exact track-level verification.

### Editorial note

The restart now has a coherent opening arc and a clear next need.
