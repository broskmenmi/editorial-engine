# GROOVE OVER NOISE — Editorial Notes

## Structural principles

- Sequence for a long-form listening journey, not a live DJ set.
- Do not assume beatmatching or harmonic mixing.
- Use concrete track anchors plus structural roles when inserting tracks.
- Prefer continuity over isolated highlights.
- Increase density before intensity.
- Let pressure accumulate through progression rather than drops.
- Preserve decompression space near the end.
- Spotify Mix may alter the intended sequence; use it for discovery, not for the canonical journey.

## Current assessment

The full arc is now established. **Psyk — Eclipse** narrows the rhythmic field, and **Luigi Tozzi — Calipso** completes the descent by dissolving pressure without removing the pulse.

## Next scouting priority

Protect the ending from overextension. Compare **Claudio PRC — Tonal** directly against Calipso as an alternative closer, but do not add both. Future candidates must replace or repair a concrete weak point rather than merely lengthen the playlist.

## Output discipline

- Purpose: maximum eight words.
- Reason: one clean sentence, maximum ten words.
- Avoid explanatory paragraphs in the user-facing report.

## Candidate verdicts

- **ADD** — belongs in the canonical ledger now.
- **REVISIT** — potentially aligned, but a specific uncertainty remains.
- **REJECT** — weakens or dilutes the playlist identity.