...
2. **Repair lane** — when an actionable repair exists, Scout ranks honest resolution leads for the highest-priority target, then evaluates only the strongest one to three unique exact candidates selected by the resolver. An exact authorized move, removal, reorder, or replacement with an already agreed and resolved track that needs no candidate search may proceed directly to Sequencer.
...
4. **Scout result** — apply Scout's objective floor to outward discoveries, rank one to nine honest resolution leads, and never pad a weak set. Resolve those leads once through the repository lifecycle; only the strongest one to three exact Spotify identities in matching `scout-data.json` become the frozen candidate shortlist. Record scan timestamp, source paths or query windows, inspected scope, exclusions, leads, and final resolved shortlist. If EXPLORE finds no lead meeting the non-identity floor, or every otherwise plausible lead is confirmed unavailable on Spotify during the completed scan, return `EXPLORATION COMPLETE — NO QUALIFIED CANDIDATES`. If REPAIR similarly finds none, return `REPAIR SEARCH COMPLETE — NO QUALIFIED CANDIDATES`. If the required scan or resolver dependency did not run, was unavailable, or produced no matching terminal snapshot, return `EXPLORATION NOT COMPLETED` or `REPAIR SEARCH NOT COMPLETED`; never misreport that as playlist completeness or repair exhaustion.
...
- When one or more discoveries meet the non-identity floor, write `scout-request.json` at most once per editorial run with `schemaVersion: 2`, the current pre-request repository SHA as `sourceCommit`, and one to nine ranked entries under `leads`. Do not write a `candidates` field in the request, and do not combine the request commit with changes to ledger, rejected, revisit, or discoveries state.
- Set `mode` to `REPAIR` or `EXPLORE`. For EXPLORE, freeze the complete exploration receipt in the request before resolution.
- For each lead preserve `searchIntent`, `proposedPlacements`, `fitHypothesis`, separated evidence fields, and an exact `spotifyTrackId` when already known. A supplied ID is still verified; no broad or alternate-version match may substitute.
- Resolve the immutable lead request into `scout-data.json` at most once per `runId`. Any semantic edit under the same `runId` anywhere in history is invalid; use a new `runId` for a genuinely new request. If resolver inputs changed after `sourceCommit`, abandon the stale request and create a new run from current state.
- A resolver-only recovery after a technical failure may use one new `runId` with `recoveryOfRunId` and `recoveryReason`. It must pin `sourceCommit` to the commit containing the failed request and preserve that request's mode, target, exploration receipt, and ranked leads exactly apart from the schema-v2 lead migration. This resumes the original documented scan; it is not a new scheduled editorial run and must not claim a second fresh scan. If canonical resolver inputs changed, recovery is forbidden and a genuinely fresh run is required.
- Recovery cannot normalize away a mutated source request. A legacy source that predates enforcement and has multiple historical contents requires `legacySalvage.reason` plus an exhaustive `legacySalvage.sourceCommits` list covering every conflicting variant. The validator rejects missing or incomplete disclosure and mechanically limits salvage to commits at or before the hard immutable-history cutoff; this exception does not authorize future same-`runId` edits.
- Freeze only the one-to-three exactly resolved `scout-data.json.candidates` before evaluation. Unresolved leads and resolved-but-unselected alternates remain diagnostic evidence, not candidates.
...
- Mark resolution `COMPLETE`, `PARTIAL`, or `NONE`. COMPLETE resolves every lead; PARTIAL resolves at least one; NONE resolves zero. PARTIAL may proceed with up to three unique selected candidates while preserving every unresolved identity, duplicate identity, exact error, and resolved-but-unselected alternate; never substitute or pad. NONE is persisted as a matching terminal diagnostic snapshot with an empty candidate list only when all identity lookups completed normally. Credentials, network, authentication, rate limits, Spotify 5xx responses, stale inputs, and malformed or conflicting snapshots remain operational `*_NOT COMPLETED` failures.
- If no lead qualifies before resolution, create no padded request or snapshot; send the documented repair or exploration receipt to the Auditor.
- After a bounded wait, require `scout-data.json.runId` to equal `scout-request.json.runId`. A stale snapshot is never evidence for the current run.
...
- **One to three evaluated candidates:** include every candidate; Verdict, Track — Artist, Position, Purpose, one-sentence Reason. When resolution is PARTIAL, also name every unresolved `Artist — Track: resolver error` without linking it.
...
- **Resolution NONE:** use the appropriate zero-qualified-candidate outcome and name every unresolved `Artist — Track: resolver error`; do not link unresolved leads.
- **Search blocked, unperformed, or no matching terminal snapshot:** `EXPLORATION NOT COMPLETED` or `REPAIR SEARCH NOT COMPLETED` plus the exact technical reason. Name each request lead and say `no matching terminal snapshot` when applicable. Never fall back to an older `scout-data.json` or claim that no suitable music or repair exists.
