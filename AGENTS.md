...
- Before the immutable request commit, write the complete request draft and current canonical ledger to temporary local files and run `node apps/spotify-scout/src/validate-proposed-placements.js <draft-request.json> <ledger.md>`. The exact draft bytes intended for commit must pass; a validation failure is `REQUEST_NOT_COMPLETED`, not an immutable resolver attempt.
